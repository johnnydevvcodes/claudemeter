...
- **Smart default budget** for live weekly rows. When no explicit `budget` is
  set, the percentage ceiling now defaults to 1.5x your busiest week on record,
  so a peak week reads ~67% (yellow) instead of a pinned red 100%, and lighter
...
- Weekly rows now correctly read **0%** when the current calendar week has no
  usage yet. Previously, if this week had no activity, the latest `ccusage`
